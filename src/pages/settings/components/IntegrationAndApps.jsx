...
const IntegrationAndApps = () => {
  ...
  const postRequest = () => {
    axios
      .post('https://api.voxclips.hng.tech/settings/add', data, {
        headers: {
          Authorization:
            'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6ImRlZmF1bEBnbWFpbC5jb20iLCJpYXQiOjE2NzA0OTc5MDQsImV4cCI6MTY3MDc1NzEwNH0.gQPUwmmHuQW7f7SEXkG5p4ypYHvdT2RwqEULDRWl10k'
        }
      })
  ...
  const handleClick = (item) => {
    ...
    if (item === 'twitter') {
      setTwitter(!twitter);
    } else if (item === 'facebook') setFacebook(!facebook);
    else if (item === 'youtube') setYoutube(!youtube);
    else if (item === 'googleDrive') setGoogleDrive(!googleDrive);
    else if (item === 'oneDrive') setOneDrive(!oneDrive);
    else if (item === 'dropBox') setDropBox(!dropBox);
    else if (item === 'instagram') setInstagram(!instagram);
    else if (item === 'linkedIn') setLinkedin(!linkedIn);
  ...
  return (
    <section className="mt-6">
      ...
      <p className="text-md w-4/5">
        You can connect and upload all your videos to your favourite softwares directly from
        Voxclips
      </p>
      <div className="grid gap-2 md:flex md:gap-4 mt-6">
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <input
              type="search"
              className="bg-[#E1E1E1] border-[#9B9D9F] border pl-10 rounded-md w-52 max-w-[259px] h-14"
