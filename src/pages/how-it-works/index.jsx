    ...
    <Layout>
      <main className='bg-[#F5F5F5]'>
        ...
        <div className={styles.heading2}> 
          ...
          <p>
            Voxclips allows podcasters to select and customise chararcters based 
            on the number of speakers in the recorded audio. Backgrounds are added 
            to create a colorful and captivating environment for viewers. 
            Convert your audio podcasts to eyecatching videos in few clicks. 
          </p>
        ...
        <div className=" container mx-auto">
          ...
          <section id="watch_video" className="px-2 md:px-2 lg:px-6 xl:px-16 2xl:px-20 flex flex-col gap-8 py-6">
            {items.map((item) => (
              <div
                key={item.id}
                className={` ${
                ...
                <div className={`flex ${(item.id === 2 || item.id === 3) ? 'md:justify-end' : ''} w-full`}>
                  <div>
                    <h5 className="py-3 md:py-4 ml-14 text-lg md:text-xl text-[#2563EB] font-bold uppercase">{item.step}</h5>
                    <div className="flex gap-3 items-start max-w-[520px]">
                      <img src={item.icon} alt="" />
                      <div className="text-[#292D32] flex flex-col gap-3">
                        <h3 className="text-[#292D32]  text-lg md:text-3xl font-semibold">{item.heading}</h3>
                        <p className="text-[#292D32]  text-sm md:text-2xl font-normal">{item.description}</p>
                      </div>
                    </div>
                  </div>
                </div>
